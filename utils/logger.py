...
class WandBOutputFormat(KVWriter):
    def __init__(self, filename, args):
        wandb.init(project=f'{args.project_name}',
                   name=filename.split('/')[-1], anonymous="allow")
        wandb.config.update(args)
        wandb.run.tags = wandb.run.tags + (args.jericho_add_wt,)
...
def make_output_format(format, ev_dir, log_suffix='', args=None):
    ...
    if format == 'stdout':
        ...
        return WandBOutputFormat(ev_dir, args)
    else:
...
def logkv_sum(key, val):
    """
    """
    Logger.CURRENT.logkv_sum(key, val)
...
def log(*args, level=INFO, condition=True):
    """
    Write the sequence of args, with no separators, to the console and output files (if you've configured an output file).
    """
    Logger.CURRENT.log(*args, level=level, condition=condition)
...
class Logger(object):
    ...
    def __init__(self, dir, output_formats, off=False):
    ...
    def log(self, *args, level=INFO, condition=True):
        if self.level <= level and condition and not self.off:
            self._do_log(args)
...
def configure(dir=None, format_strs=None, off=False):
    if dir is None:
    ...
    Logger.CURRENT = Logger(dir=dir, output_formats=output_formats, off=off)
